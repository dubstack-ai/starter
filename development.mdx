...
<Accordion title="Error: Cannot find module 'sharp'">
  ...
  **Solution:**
  - Remove the CLI: `npm remove -g @dubstack/cli`
  - Update to Node.js 18 or higher
  - Reinstall: `npm install -g @dubstack/cli`
</Accordion>
...
<Accordion title="Changes not appearing">
  ...
  **Solutions:**
  ...
  - Restart the dev server: `Ctrl+C` then `dubstack dev`
  - Clear your browser cache: `Cmd+Shift+R` (Mac) or `Ctrl+Shift+R` (Windows)
...
<Accordion title="Images not loading">
  ...
  **Solution:**
  - Place images in `/images` directory at repository root
  - Reference them with absolute paths: `/images/hero.png`
  - Ensure image file names don't have spaces
</Accordion>
</AccordionGroup>
